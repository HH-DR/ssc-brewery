...
//can´t use nested tests bug - https://github.com/spring-projects/spring-security/issues/8793
...
//    class createOrderTests {
    ...
    void listOrdersCustomerAuth() throws Exception {
        ...
        log.debug("listOrdersCustomerAuth: " + principal.getCustomer());
        mockMvc.perform(get(API_ROOT + dunedinCustomer.getId() + "/orders"))
                .andExpect(status().isOk());
    }
    ...
    @Transactional
//    wenn man nicht @Transactional macht, dann kommt der folgende Fehler. Wieder einmal JPA und Spring Security, dass nicht dem Context von JPA läuft.
//    man geht von Customer zu einem BeerOrder-Object ausserhalb eines transactional context
//    man kann 1. @Transactional oder 2. die Konfiguration auf EAGER umstellen
//    org.hibernate.LazyInitializationException: failed to lazily initialize a collection of role: guru.sfg.brewery.domain.Customer.beerOrders, could not initialize proxy - no Session
    @Test
